...
현재 구현은 산-염기, 산소화, 네프론 전해질 수송, 심장 혈역학, ECG·부정맥, 내분비 호르몬 축이다. 하나의 기초 개념이 여러 분과와 관련되면 같은 페이지를 여러 Overview에서 공유하며 관련 질병과 검사 페이지에서도 들어올 수 있다.
...
- 네프론 모델은 분절별 정상 여과 부하 처리 비율을 기준으로 하며, 이뇨제는 해당 수송체가 있는 분절에서만 flux를 바꾼다. 음수 처리량은 재흡수가 아니라 분비임을 범례와 설명에 표시한다.
