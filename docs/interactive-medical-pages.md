...
| 역할 | 경로 |
| --- | --- |
| 페이지 메타데이터와 연결 대상 | `src/lib/interactive-concepts.ts` |
...
1. `interactive-concepts.ts`에 slug, 표시명, 요약, 관련 분과와 연결 대상을 등록한다.
...
6. 등록한 분과 Overview, 질병, 검사, 약물 페이지에 역방향 링크가 나타나는지 확인한다.
