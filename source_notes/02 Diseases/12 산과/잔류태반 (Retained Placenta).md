---
aliases:
- 잔류태반
유형:
- disease
계통: 산과
분류:
- 산과적 출혈
CC:
- 출혈
preprocessing_required: x
last_studied: '2026-06-06'
---
분과 : [[산과]]

## 1. 개요
- **정의**: 분만 후 태반의 일부 또는 전부가 자궁 내에 남아있는 상태

- **원인**
  - 태반의 불완전한 박리
  - 유착태반(Placenta accreta spectrum) 등 태반 부착 이상
  - 자궁 수축 부전

- **기전**: 태반이 완전히 배출되지 않아 자궁의 수축을 방해하고, 자궁 내막의 혈관이 노출된 상태로 남아 지속적인 출혈을 유발함



## 2. 임상 양상 (Clinical Features)
* **CC**: 분만 후 지속적인 질 출혈
* **P/E**
  - 자궁이 단단하게 수축되지 않고 이완되어 있음
  - 하복부 압통

## 3. 검사 (Lab & Imaging)
* 초음파 검사(US): 자궁 내 잔류 태반 조직 확인

## 4. 진단 (Diagnosis)
* 임상적 증상(지속적 출혈)과 초음파 소견을 종합하여 진단

## 5. 치료 (Treatment)
- 자궁 수축제(Oxytocin 등) 투여
- 자궁 내 잔류물 제거술(Gentle suction curettage)
- 필요 시 자궁 내막 소파술
- 심한 경우 수술적 덩이 제거술

## 6. 예후 및 합병증 (Prognosis)
- 산후 출혈(Postpartum hemorrhage) 및 감염 위험 증가
- 적절한 처치 시 예후 양호

출처 : StatPearls (Embryology, Placenta; Physiology, Placenta)
Last updated :
---
## 통합 전 원문
---
aliases: ["placenta retention"]
유형:
- disease
계통: "산과"
분류: "["산과", "산과적 출혈"]"
CC: []
---
## 1. 개요
- **정의**
- **원인**
- **기전**
## 2. 임상 양상 (Clinical Features)
* **CC** 분만 후 지연 출혈.
* **P/E**
## 3. 검사 (Lab & Imaging)
* US (초음파)에서 보임.
## 4. 진단 (Diagnosis)
## 5. 치료 (Treatment)
* Gentle suction curettage.
* Oxytocin.
* 자궁내막 소파술.
* 덩이 제거술.
## 6. 예후 및 합병증 (Prognosis)
출처 : 
Last updated :
---
## Merged source from 11 외과
---
aliases: ["placenta retention"]
유형:
- disease
계통: "외과"
분류: "["외과", "산과", "산과적출혈"]"
CC: []
---
## 1. 개요
- **정의**
- **원인**
- **기전**
## 2. 임상 양상 (Clinical Features)
* **CC**: 분만후지연출혈(US에서보임)
* **P/E**
## 3. 검사 (Lab & Imaging)
## 4. 진단 (Diagnosis)
## 5. 치료 (Treatment)
- gentle suction curettage, oxytocin, 자궁내막소파술, 덩이제거술
## 6. 예후 및 합병증 (Prognosis)
출처 : 
Last updated :
2026-05-18 — 본문 갱신 (codex fill)
