---
content_updated_at: "2026-07-30"
aliases:
- deep vein thrombosis
- DVT
유형:
- disease
계통: 산과
분류:
- 임신 중 질환
CC:
- 부정맥
- 부종
preprocessing_required: x
last_studied: '2026-06-06'
document_role: "special_population"
display_title: "임신 중 심부정맥혈전증"
parent_disease: "심부 정맥 혈전증 (Deep Vein Thrombosis)"
relation_to_parent: "special_population"
population: "pregnancy"
clinical_priority: "tier_3"
review_status: "tier3_full_flash_35_enriched"
reviewed_at: "2026-07-30"
---
분과 : [[산과]]

## 1. 개요
- **정의**: 심부정맥(주로 하지) 내 혈전이 형성되어 혈류를 방해하는 질환.
- **원인**: Virchow의 삼요소(정맥 울혈, 혈관 내피 손상, 과응고 상태)에 의해 발생.
- **기전**:
  혈류 정체 및 내피 손상으로 인한 혈전 형성 및 파급.
  폐색전증(PE)의 주요 원인.

## 2. 임상 양상 (Clinical Features)
* **CC**: 하지 부종, 통증, 압통, 발적, 열감.
* **P/E**: 일측성 하지 부종, 표재 정맥 확장, Homan's sign(비특이적).

## 3. 검사 (Lab & Imaging)
- **D-dimer**: 민감도가 높아 선별 검사로 유용하나 특이도는 낮음.
- **초음파(Ultrasound)**: 근위부 심부정맥 혈전 진단을 위한 1차 영상 검사(압박 초음파).
- **Wells score**: 임상적 확률 평가를 통해 검사 및 치료 전략 결정.

## 4. 진단 (Diagnosis)
- Wells score를 통한 위험도 계층화 후 D-dimer 및 초음파 시행.
- 임상적 의심이 높을 경우 즉각적인 영상 검사 및 항응고 요법 고려.

## 5. 치료

* **항응고 요법**: 임신 중에는 태반 통과가 없는 LMWH(저분자량 헤파린)를 우선적으로 사용하며, 출산 전후 중단 시기를 고려하여 관리한다.
* **폐색전증 예방**: 급성기에는 혈전의 파급 및 폐색전증 방지를 위해 절대 안정과 함께 즉각적인 항응고 치료를 시행한다.
* **물리적 예방**: 고위험군 산모는 압박 스타킹이나 간헐적 공기 압박 장치(IPC)를 사용하여 정맥 울혈을 방지한다.
* **다학제 협진**: 고위험 임신부의 경우 산과, 혈액내과, 흉부외과 등 다학제적 접근을 통해 항응고제 용량 조절 및 분만 계획을 수립한다.
* **추적 관찰**: 치료 종료 후에도 재발 여부와 혈전후 증후군 발생을 정기적으로 모니터링한다.

## 6. 예후 및 합병증 (Prognosis)
- **합병증**: 폐색전증(PE), 혈전후 증후군(Post-thrombotic syndrome).
- **예후**: 조기 진단 및 적절한 항응고 치료 시 사망률 및 이환율 감소.

Last updated :
2026-05-20 — 본문 갱신 (codex fill)
