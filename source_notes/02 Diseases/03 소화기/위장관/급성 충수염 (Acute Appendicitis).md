---
aliases:
- acute appendicitis
유형:
- disease
계통: 소화기
분류:
- 하부위장관
CC:
- 복통
preprocessing_required: x
last_studied: '2026-06-06'
clinical_priority: "tier_2"
reviewed_at: "2026-07-16"
review_status: "guideline_enriched"
guideline_year: "2026"
sources: ["Diagnosis and Treatment of Acute Appendicitis: 2025 Edition of the World Society of Emergency Surgery Jerusalem Guidelines. | https://pubmed.ncbi.nlm.nih.gov/41604201/"]

content_updated_at: "2026-07-16"
---
분과 : [[외과]]

## 1. 개요
- **정의**: 충수(appendix)의 내강이 폐쇄되어 발생하는 급성 염증성 질환.
- **원인**: 충수 내강의 폐쇄(분변석, 림프구 증식, 이물질 등)가 가장 흔하며, 이로 인한 세균 증식과 점막 손상이 염증을 유발함.
- **기전**: 내강 폐쇄 → 내강 내 압력 상승 → 혈류 장애 → 허혈 및 점막 괴사 → 천공 및 복막염으로 진행.

## 2. 임상 양상 (Clinical Features)
* **CC**: 초기에는 모호한 상복부 또는 배꼽 주위 통증(visceral pain)으로 시작하여, 점차 우하복부(RLQ)로 국소화되는 통증(somatic pain).

* **P/E**:
  우하복부 압통(McBurney's point tenderness), 반동 압통(rebound tenderness), 근육 방어(guarding).
  발열, 오심, 구토가 동반될 수 있음.

## 3. 검사 (Lab & Imaging)
- **진단 보조**: 임상 위험 점수(clinical risk scores) 및 영상 검사는 진단 정확도를 높이고 불필요한 충수절제술(negative appendectomy) 비율을 낮춤.
- **Imaging**: 복부 CT가 표준이나, 환자군(소아, 임산부 등)에 따라 초음파 등 적절한 영상 기법을 선택함.
- **주의**: 영상 검사 및 임상 점수는 진단 표준화와 불확실성 감소를 위해 활용함.
- **진찰 보완**: 후맹장 또는 골반 내 충수염에서는 전형적 우하복부 압통이 뚜렷하지 않을 수 있어, 병력과 복부 진찰을 영상 소견과 함께 해석한다.

## 4. 진단 (Diagnosis)
- 전형적인 임상 증상과 신체 검진을 바탕으로 진단하며, 불확실한 경우 CT 촬영을 통해 확진함.

## 5. 치료 (Treatment)
- **수술**: 복강경 충수절제술(Laparoscopic appendectomy)이 표준 치료임.
- **수술 시기**: 단순 충수염(uncomplicated)의 경우, 24시간 이내의 수술 지연은 부작용 위험을 증가시키지 않음.
- **비수술적 치료**: 선택된 단순 충수염 환자에서 항생제 치료는 안전하고 효과적인 대안임.
- **복잡성 충수염(Complicated)**: 농양 동반 시 비수술적 치료 후 종양 배제를 위한 추적 관찰이 필수적이며, 수술 후 항생제는 2-3일의 단기 요법으로 제한함.
- **농양**: 국한된 충수주위 농양에서 배액이 필요하면 영상 유도 배액을 고려하며, 환자 상태와 해부학적 접근 가능성을 바탕으로 외과팀과 치료 전략을 정한다.

## 6. 예후 및 합병증 (Prognosis)
- 조기 진단 및 수술 시 예후가 좋음.
- 합병증: 천공, 복막염, 농양 형성, 장폐색.

출처 : StatPearls (Acute Abdomen); 삼성서울병원 외과 메뉴얼
Last updated :
2026-05-18 — 본문 갱신 (codex fill)
