---
aliases: []
유형:
- disease
계통: 내분비
분류:
- 지질
CC:
- 무증상
preprocessing_required: x
last_studied: '2026-06-06'
clinical_priority: "tier_2"
reviewed_at: "2026-07-30"
review_status: "guideline_enriched"
guideline_year: "2026"
sources: ["Update on familial hypercholesterolemia: An expert clinical consensus from the National Lipid Association (2026) | https://pubmed.ncbi.nlm.nih.gov/41741298/", "2022 Consensus statement on the management of familial hypercholesterolemia in Korea | https://www.kjim.org/journal/view.php?number=22121"]

content_updated_at: "2026-07-30"
---
분과 : [[내분비]]

## 1. 개요
- **정의**: LDL 수용체, ApoB, 또는 PCSK9 유전자 변이로 인해 LDL 콜레스테롤 제거율이 감소하여 혈중 LDL-C 농도가 심각하게 상승하는 유전 질환

- **원인**: 상염색체 우성 유전(주로 LDL 수용체 유전자 변이).

- **기전**: LDL 수용체 기능 장애로 간세포의 LDL-C 섭취가 감소하여 혈중 LDL-C 농도가 상승하고, 이로 인해 조기 죽상경화증 및 심혈관 질환 위험이 증가한다.

## 2. 임상 양상 (Clinical Features)
* **CC**: 조기 관상동맥 질환(premature CHD) 가족력, 고콜레스테롤혈증.

* **P/E**
  - 건 황색종(tendon xanthoma): 아킬레스건이나 손가락 신전건에 흔함.
  - 각막 환(corneal arcus): 45세 이전 발생 시 의심.
  - 황색판종(xanthelasma) 및 결절성 황색종(tuberous xanthoma).

## 3. 검사 (Lab & Imaging)
- **혈액 검사**: 공복 지질 검사(LDL-C 상승, 중성지방은 대개 정상).
- **유전자 검사**: 진단 확진, 가족 내 선별 검사(cascade screening) 촉진 및 진단 불확실 시 시행.
- **기타**: 심장 초음파 및 혈관 영상(동맥경화 평가 및 위험도 계층화).
출처: Update on familial hypercholesterolemia: An expert clinical consensus from the National Lipid Association.

## 4. 진단 (Diagnosis)
- 임상적 진단 기준(Dutch Lipid Clinic Criteria 등)을 주로 사용하며, LDL-C 수치, 가족력, 황색종 유무 등을 종합하여 평가한다.
- 1차 친족에 대한 선별 검사(Cascade screening)가 필수적이다.

## 5. 치료 (Treatment)
- **가족 단위로 시작**: 조기 ASCVD 위험을 설명하고, 1·2촌 혈족 cascade screening과 소아 평가를 연계한다. 생활요법은 필요하지만 LDL 수용체 결함에 대한 약물치료를 대체하지 않는다.
- **이형접합 FH**: 최대 내약 용량 고강도 statin을 시작하고 LDL-C 목표·위험도에 따라 ezetimibe, PCSK9 억제제/siRNA 또는 bempedoic acid를 순차 추가한다. 임상 ASCVD·매우 높은 LDL-C는 조기 전문의 의뢰 대상이다.
- **동형접합 FH**: lipid specialist 의뢰가 필요하며, LDL apheresis와 lomitapide·evinacumab 등 치료 접근성을 포함한 다중요법을 검토한다.
- **안전·특수상황**: 임신 계획·임신 중에는 statin 등 금기/제한 약제를 사전에 조정하고, 간효소·근육 증상·순응도와 LDL 반응을 추적한다.: 저콜레스테롤 식이, 체중 조절, 금연.
- **약물 치료 단계적 접근**:
  1. 1차: 최대 내약 용량 스타틴(statin) 및 에제티미브(ezetimibe).
  2. 2차: PCSK9 억제제, 벰페도익산(bempedoic acid) 추가 고려.
  3. 동형접합(Homozygous) FH: 로미타피드(lomitapide), 에비나쿠맙(evinacumab), 지질 성분 분리술(lipoprotein apheresis) 고려.
- **치료 목표**: 심혈관 질환 예방을 위해 강화된 LDL-C 목표치 설정 및 지속적 모니터링.
출처: Update on familial hypercholesterolemia: An expert clinical consensus from the National Lipid Association.

## 6. 예후 및 합병증 (Prognosis)
- 조기 진단 및 적극적인 LDL-C 저하 치료를 통해 관상동맥 죽상경화증의 진행을 늦추거나 예방할 수 있다.

Last updated :
2024-07-30 — 초기 작성
2026-05-18 — 본문 갱신 (codex fill)
