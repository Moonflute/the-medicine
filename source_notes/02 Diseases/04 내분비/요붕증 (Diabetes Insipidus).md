...
## 3. 검사 (Lab & Imaging)
- **기본 검사**: 24시간 소변량으로 실제 다뇨를 먼저 확인한 뒤 혈청·소변 삼투압, 전해질과 glucose를 함께 평가하여 삼투 이뇨와 수분 이뇨를 구분한다. 소변 삼투압 < 200 mOsm/kg, 비중 < 1.005는 수분 이뇨를 시사한다.
- **특수 검사**: 수분 제한 검사(엄격한 감독 하 시행).
- 부분 요붕증과 원발 다음증은 검사 결과가 겹칠 수 있다. 기저 AVP 수치만으로 확정하지 않으며, 고나트륨혈증·탈수가 있으면 수분 제한 검사를 진행하지 않고 먼저 상태를 평가·안정화한다.
- **영상 및 추가 검사**: 뇌 MRI(시상하부-뇌하수체 확인). 특발성 뇌하수체 줄기 비대나 중추성 요붕증 환자에서는 악성 종양 배제를 위해 혈청 β-hCG, 알파태아단백(AFP), 흉부 X-선, 복부 초음파, 안과 검진 및 골격계 조사를 고려함.
