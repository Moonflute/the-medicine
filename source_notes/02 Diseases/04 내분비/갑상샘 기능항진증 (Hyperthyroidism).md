...
## 3. 검사
- **혈액 검사**: 혈청 TSH 수치 저하(가장 민감한 지표), 유리 T4 및 T3 수치 상승. TSH가 낮지만 유리 T4가 정상인 경우에는 T3를 측정하여 T3 우세 갑상샘중독증 여부를 확인한다.
...
- 원인이 불분명하면 호르몬 합성 증가 상태와 갑상샘염에 의한 저장 호르몬 유리 또는 외인성 호르몬 섭취를 구분한다. RAIU와 혈류 평가는 이 감별에 사용하되 임신·수유와 최근 요오드 노출 여부를 함께 고려한다.
- 그레이브스 안병증에서 시력 저하, 색각 변화, 시야 이상 또는 각막 노출이 있으면 시력 위협 소견으로 신속한 안과 평가가 필요하다.
...
- **항갑상샘제 안전성 평가**: 시작 전 CBC와 간기능을 확인한다. 발열 또는 인후통이 새로 발생하면 약을 중단하고 즉시 CBC로 무과립구증을 평가하며, 정기 CBC만으로 갑작스럽게 발생하는 무과립구증을 예방할 수는 없다.
