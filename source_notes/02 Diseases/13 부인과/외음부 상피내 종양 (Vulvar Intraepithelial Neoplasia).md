---
content_updated_at: "2026-07-30"
aliases:
- VIN
- vulvar intraepithelial neoplasia
- 외음부 상피내 종양
유형:
- disease
계통: 부인과
분류:
- 부인과 종양
- 질·외음부 종양
CC:
- 가려움증
preprocessing_required: x
last_studied: '2026-06-06'
clinical_priority: "tier_3"
review_status: "tier3_full_flash_35_enriched"
reviewed_at: "2026-07-30"
---
분과 : [[부인과]]

## 1. 개요
- **정의**: 외음부의 비침습적 편평상피 병변으로, 외음부 편평세포암의 전암 병변임.
- **원인**: 크게 두 가지 유형으로 분류됨.
  - uVIN (usual type): HPV 감염과 관련 (주로 HPV-16).
  - dVIN (differentiated type): 만성 염증(태선경화증 등)과 관련.
- **기전**:
  HPV 감염 시 E6, E7 oncoprotein이 세포 기능을 방해함.
  dVIN은 만성 산화적 유전 손상에 의해 발생.

## 2. 임상 양상 (Clinical Features)
* **CC**: 소양증, 작열감, 통증, 외음부의 백색 또는 붉은색 병변.
* **P/E**:
  병변은 다발성일 수 있으며, 융기되거나 평평한 형태, 백색, 붉은색, 회색 또는 색소 침착을 보임.
  특이적인 외형이 없으므로 의심되는 모든 병변은 생검이 필수적임.

## 3. 검사 (Lab & Imaging)
- **생검**:
  진단을 위한 필수 검사.
  병변의 위치, 크기, 개수 등을 기록.
- **기타**:
  육안적 검사 및 필요 시 확대경(colposcopy) 사용.
  아세트산 도포는 특이도가 낮아 권장되지 않음.

## 4. 진단 (Diagnosis)
- 임상적 진단 후 생검을 통해 확진.
- 분류 (2015 ISSVD):
- LSIL (Low-grade SIL): 이전의 VIN 1.
- HSIL (High-grade SIL): 이전의 VIN 2/3 (uVIN).
- dVIN (Differentiated VIN): 이전의 VIN simplex type.

## 5. 치료

- **병변 제거**: uVIN은 국소 절제술을 우선 고려하며, dVIN은 침습성 암으로의 높은 진행 위험을 고려하여 광범위 국소 절제술(Wide local excision)을 시행함.
- **기능 보존**: 치료 시 외음부의 해부학적 구조와 기능 보존을 최우선으로 고려하며, 병변의 완전 절제와 조직학적 확인을 병행함.
- **보조 요법**: uVIN의 경우 수술적 치료가 어려운 경우 면역 조절제(Imiquimod) 사용을 고려할 수 있음.
- **추적 관찰 및 예방**: 높은 재발률을 고려하여 정기적인 외음부 검진을 시행하며, HPV 백신 접종과 금연을 권장함.

## 6. 예후 및 합병증 (Prognosis)
- 치료 후에도 높은 재발률을 보임.
  치료하지 않을 경우 외음부암으로 진행 가능.
- 장기적인 추적 관찰(6~12개월 간격)이 필요함.

출처 : StatPearls (Vulvar Intraepithelial Neoplasia)
Last updated :
2026-07-15 — 중복 본문 정리 (Tier 3)
