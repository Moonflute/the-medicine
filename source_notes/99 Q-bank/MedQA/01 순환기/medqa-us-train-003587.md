---
type: qbank
schema_version: 1
id: medqa-us-train-003587
source: MedQA-US
source_split: train
source_meta: step2&3
source_hash: sha256:a512862ffcb6b88849722203d59225a12f9071c02cc2c85695fd38e46a5b10fb
exam: USMLE Step 2/3
language: ko
specialty: 01 순환기
related_diseases:
  - "deep vein thrombosis"
  - "acute lower extremity DVT"
  - "anticoagulation"
  - "heparin"
related_disease_slugs:
  - MDEg7Iic7ZmY6riwL-yLrOu2gCDsoJXrp6Ug7ZiI7KCE7KadIChEZWVwIFZlaW4gVGhyb21ib3NpcykubWQ
  - MDkg7ZiI7JWhL-2XpO2MjOumsCDsnKDrsJwg7ZiI7IaM7YyQ6rCQ7IaM7KadIChIZXBhcmluLUluZHVjZWQgVGhyb21ib2N5dG9wZW5pYSwgSElUKS5tZA
question_type: management
difficulty: complex
answer: B
translation_status: machine-verified
explanation_status: machine-generated
translation_model: codex-direct
translation_prompt_version: codex-direct-ko-v1
translated_at: 2026-07-18
review_status: machine-verified
explanation_model: openai/gpt-oss-120b
explanation_prompt_version: explanation-ko-v1
related_drug_slugs:
  - ZHJ1ZzowMSDsi6ztmIjqs4QvQXNwaXJpbi5tZA
  - ZHJ1ZzowNyDrqbTsl63Ct-yXvOymncK366WY66eI7Yuw7IqkL0lidXByb2Zlbi5tZA
  - ZHJ1ZzowOSDtmIjslaHCt-ydkeqzoC9IZXBhcmluLm1k
  - ZHJ1ZzowOSDtmIjslaHCt-ydkeqzoC9XYXJmYXJpbi5tZA
---

# MedQA US 임상문제

## 문제

25세 남성이 갑자기 시작된 다리 통증으로 내원했다. 오른쪽 다리는 무릎오금 아래가 붉고 부어 있으며 압통이 있었다. 이전에도 비슷한 증상이 있었지만 아세트아미노펜으로 며칠 후 호전되었다. 다음 중 가장 적절한 치료는?

## 선택지

A. 아스피린
B. 헤파린
C. 이부프로펜과 안정
D. 와파린

## 해설


다리의 급성 통증, 발적, 부종, 압통은 급성 심부정맥 혈전증(DVT)의 전형적인 징후이며, 치료는 즉시 항응고가 필요하다. 헤파린은 빠른 작용을 보이는 초기 항응고제로, 급성 DVT에 가장 적절한 치료이다. 따라서 헤파린 투여가 정답이다.

## 출처

- MedQA-US (GBaker/MedQA-USMLE-4-options, CC BY 4.0)
- 원본 ID: medqa-us-train-003587
