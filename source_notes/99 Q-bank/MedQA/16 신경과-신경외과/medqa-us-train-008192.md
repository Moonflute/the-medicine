---
type: qbank
schema_version: 1
id: medqa-us-train-008192
source: MedQA-US
source_split: train
source_meta: step2&3
source_hash: sha256:b86401af1f3d5000703cb7d93694a26c5df34c44fbaa9ee0b430fce07d91afaa
exam: USMLE Step 2/3
language: ko
specialty: 16 신경과-신경외과
related_diseases:
  - "subdural hematoma"
  - "bridging vein rupture"
  - "warfarin-associated bleeding"
question_type: mechanism
related_disease_slugs:
  - MTYg7Iug6rK96rO8LeyLoOqyveyZuOqzvC_rp4zshLEg6rK966eJ7ZWYIOy2nO2YiCAoQ2hyb25pYyBzdWJkdXJhbCBoZW1hdG9tYSkubWQ
difficulty: complex
answer: D
translation_status: machine-verified
explanation_status: machine-generated
translation_model: codex-direct
translation_prompt_version: codex-direct-ko-v1
translated_at: 2026-07-18
review_status: machine-verified
explanation_model: openai/gpt-oss-120b
explanation_prompt_version: explanation-ko-v1
related_drug_slugs:
  - ZHJ1ZzowMSDsi6ztmIjqs4QvSHlkcm9jaGxvcm90aGlhemlkZS5tZA
  - ZHJ1ZzowMSDsi6ztmIjqs4QvTGlzaW5vcHJpbC5tZA
  - ZHJ1ZzowMSDsi6ztmIjqs4QvTWV0b3Byb2xvbC5tZA
  - ZHJ1ZzowOSDtmIjslaHCt-ydkeqzoC9XYXJmYXJpbi5tZA
---

# MedQA US 임상문제

## 문제

68세 여성이 실신으로 넘어지고 난 후 의식저하, 두통, 오심이 발생해 응급실로 이송되었다. 고혈압과 심방세동이 있으며 히드로클로로티아지드, 리시노프릴, 메토프롤롤, 와파린을 복용한다. 혈압 140/90 mm Hg, 맥박 83회/분으로 불규칙하고 호흡수 12회/분, 체온 36.8°C(98.4°F)이다. 의식은 있으나 혼돈되어 있고 운동 지시에 따르며 양측 동공은 둥글고 같지만 빛에 반응이 약하다. 양쪽 눈 외전이 불가능하고 우측 상·하지 근력저하와 근긴장·반사가 증가되어 있다. 두부 CT가 제시되어 있다. 이 환자 상태를 일으킨 원인은?

## 선택지

A. 중경막동맥 파열
B. 경동맥 순환 부위 낭상동맥류 파열
C. 연질막 혈관 열상
D. 뇌 연결정맥 파열

## 해설


두부 CT에서 뇌 표면을 연결하는 정맥(bridging veins)이 파열된 출혈이 보이며, 와파린 복용으로 출혈 위험이 증가한다. 정맥 파열은 서브두개혈종을 일으키며, 환자의 혼돈, 국소 신경학적 결손과 일치한다. 따라서 원인은 뇌 연결정맥 파열이다.

## 출처

- MedQA-US (GBaker/MedQA-USMLE-4-options, CC BY 4.0)
- 원본 ID: medqa-us-train-008192
