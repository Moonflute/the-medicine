...
let renderer,observer,model,disposed=false,visible=true,drag=null,resumeAt=0,frame=0,last=0;
const scene=new T.Scene(),camera=new T.PerspectiveCamera(33,1,.1,30);
camera.position.set(0,.6,6.6);camera.lookAt(0,-.15,0);
function open(){if(window.parent===window){location.assign('../atlas/?organ=heart');return}window.parent.postMessage({type:'atlas:open'},location.origin)}
host.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}});
host.addEventListener('pointerdown',e=>{if(!e.isPrimary)return;drag={id:e.pointerId,x:e.clientX,y:e.clientY,last:e.clientX,moved:false};host.setPointerCapture(e.pointerId);resumeAt=Infinity});
...
host.addEventListener('pointerup',e=>{if(!drag||drag.id!==e.pointerId)return;const click=!drag.moved&&Math.hypot(e.clientX-drag.x,e.clientY-drag.y)<7;drag=null;resumeAt=performance.now();if(click)open();start()});
host.addEventListener('pointercancel',()=>{drag=null;resumeAt=performance.now();start()});
host.addEventListener('lostpointercapture',()=>{if(drag){drag=null;resumeAt=performance.now();start()}});
function render(){if(renderer&&!disposed)renderer.render(scene,camera)}
function tick(t){frame=0;if(disposed||!visible||document.hidden)return;if(t-last>=1000/30){const dt=Math.min((t-last)/1000,.05);last=t;if(model&&!reduced.matches&&!drag){const progress=Math.min(Math.max((t-resumeAt)/600,0),1);const ease=progress*progress*(3-2*progress);model.rotation.y+=dt*.16*ease;}render()}if(!reduced.matches||drag)frame=requestAnimationFrame(tick)}
