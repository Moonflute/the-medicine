...
import {resolvePathologyStage} from './pathology-stages.js';
...
export function compilePathologyStates(states,normalComparison=false){
 ...
 return Object.entries(states).flatMap(([organId,state])=>{const base=pathology[organId]?.find(s=>s.id===state.id),scenario=state.stageId?resolvePathologyStage(base,state.stageId):base;return scenario?[{organId,state,scenario,target:new RegExp(scenario.target)}]:[];});
}
