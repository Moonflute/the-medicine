...
heart:'4개 심방/심실·4개 판막·심실중격·유두근과 주요 관상혈관 포함. 기본 모델에는 건삭·전도계, 회선가지 등 일부 분지와 미세분지가 없으며 별도 확대 모형에서 보완합니다.',
...
coverage.nose='BodyParts3D 원본 15개 구조. 비강 바닥의 위턱뼈·입천장뼈는 옅게 표시하며 선택하면 선명해집니다. 벌집뼈를 포함하며 중간·위코선반은 별도 메시로 분할되지 않았습니다. 비강 점막·부비동 공간은 미포함.';coverage.ears='OpenEar ZETA 원본 12개 구조. 고막·이소골·내이·신경·혈관의 상대 배치를 보존했습니다. 전정·반고리관은 Scala vestibuli와 한 메시이며, 귓바퀴는 상세 메뉴에서 별도 BodyParts3D 표본으로 볼 수 있습니다. 귀인두관·막미로는 미포함입니다.';delete coverage.placenta;

coverage.gallbladder='HRA 원본 담낭·담도 6개 구조. 원본 좌표와 정점 보존 검증 완료.';coverage.larynx='HRA 원본 후두 연골·근육 18개 구조. 원본 좌표와 정점 보존 검증 완료. 상세 메뉴에서 BodyParts3D 후두 인대·막 및 인두 조합을 별도로 탐색할 수 있습니다. 성대 점막층은 미포함.';

coverage.thyroid='BodyParts3D 7개 원본 구조. 변환 전후 정점·상대 위치 검증. 상세 메뉴에서 같은 출처의 후두와 함께 탐색할 수 있습니다. 내부 점막층 및 미세구조는 미포함.';
coverage.adrenals='BodyParts3D 2개 원본 구조. 변환 전후 정점·상대 위치 검증. 상세 메뉴에서 같은 출처의 후두와 함께 탐색할 수 있습니다. 내부 점막층 및 미세구조는 미포함.';
coverage.testes='BodyParts3D 4개 원본 구조. 변환 전후 정점·상대 위치 검증. 상세 메뉴에서 같은 출처의 후두와 함께 탐색할 수 있습니다. 내부 점막층 및 미세구조는 미포함.';
coverage.pharynx='BodyParts3D 13개 원본 구조. 변환 전후 정점·상대 위치 검증. 상세 메뉴에서 같은 출처의 후두와 함께 탐색할 수 있습니다. 내부 점막층 및 미세구조는 미포함.';
coverage.adrenals += " 설정에서 피막·피질 3층·수질의 별도 모식 단면을 선택할 수 있습니다.";
coverage.testes += " 설정에서 고환·부고환의 관 연결을 설명하는 별도 모식 단면을 선택할 수 있습니다.";




coverage.thyroid='BodyParts3D 원본 17개 구조: 갑상선·부갑상선과 10개 혈관 표면. 양쪽 아래갑상동맥, 오른쪽 위갑상동맥 줄기·샘 분지 및 정맥을 포함합니다. 아래갑상동맥의 말단 연결은 원본에 완전하게 제공되지 않습니다. 왼쪽 위갑상동맥·정맥망과 신경·여포는 미포함입니다.';



coverage.adrenals='BodyParts3D 부신 2개와 혈관 6개 원본 표면. 양쪽 중간·아래부신동맥 및 부신정맥을 포함합니다. 위부신동맥과 미세 분지는 미포함입니다. 피질·수질 5개 층은 별도의 모식 단면이며 원본 내부 분할이 아닙니다.';
