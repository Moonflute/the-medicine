...
let revisionIndex;
function revision(url){revisionIndex??=new Map(Object.entries(assetRevisions).map(([path,value])=>[absolute(path),value]));return revisionIndex.get(absolute(url));}
...
function validatedPlan(paths){
 const urls=[...new Set(paths)];
 if(!urls.length)throw Error('저장할 모델 파일이 없습니다.');
 for(const url of urls){if(new URL(absolute(url)).origin!==location.origin||!revision(url))throw Error('저장할 수 없는 모델 주소입니다.');}
 return {urls,bytes:urls.reduce((sum,url)=>sum+revision(url).bytes,0)};
}
export async function modelSavePlan(view){
 ...
 return validatedPlan(urls);
}
export function allModelSavePlan(){
 return validatedPlan(Object.keys(assetRevisions).filter(url=>url.startsWith('./models/current/')||url.startsWith('./imaging/')));
}
export async function saveModels(plan,{signal,onProgress=()=>{},concurrency=1}={}){
 if(!localModelStorageAvailable())throw Error('이 브라우저에서는 기기 저장을 사용할 수 없어요.');
 const cache=await caches.open(CACHE),staleByURL=new Map();
 for(const request of await cache.keys()){
  const url=new URL(request.url);url.searchParams.delete('atlas-sha256');
  const old=staleByURL.get(url.href)||[];old.push(request);staleByURL.set(url.href,old);
 }
 let completed=0,bytes=0,downloadedBytes=0,cached=0,cursor=0,failure;
 onProgress({completed,total:plan.urls.length,bytes,downloadedBytes,cached});
 async function worker(){
  while(cursor<plan.urls.length&&!failure){
   ...
   try{
    ...
    if(!await cache.match(cacheKey)){
     ...
     downloadedBytes+=rev?.bytes||0;
    }else cached++;
    for(const old of staleByURL.get(absolute(url))||[])if(old.url!==cacheKey)await cache.delete(old);
    bytes+=rev?.bytes||0;onProgress({completed:++completed,total:plan.urls.length,bytes,downloadedBytes,cached});
   }catch(error){failure??=error;break;}
  }
 }
 await Promise.all(Array.from({length:Math.min(Math.max(1,Math.floor(concurrency)||1),3,plan.urls.length)},worker));
 if(failure)throw failure;
 return {completed,bytes,downloadedBytes,cached};
}
