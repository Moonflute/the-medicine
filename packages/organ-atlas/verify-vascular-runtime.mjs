import {vesselRegions} from './src/body-vessel-catalog.js';
...
import {loadVascularDetail} from './src/body-vessel-detail.js';import {disposeDetailModel} from './src/detail-resources.js';
...
for(const region of vesselRegions){
 const root=await loadVascularDetail(region.id),frame=root.userData.detail.sourceFrame;
 const actual=new Map();root.traverse(m=>{if(m.isMesh)actual.set(m.userData.detail.sourcePartId,m)});
 for(const file of ['body-skeleton-light.glb.gz',region.file]){
