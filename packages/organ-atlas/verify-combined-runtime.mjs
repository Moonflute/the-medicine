import {combinedRegions} from './src/body-combined-catalog.js';
...
import {loadCombinedDetail} from './src/body-combined-detail.js';import {disposeDetailModel} from './src/detail-resources.js';
...
for(const region of combinedRegions){
 const root=await loadCombinedDetail(region.id),frame=root.userData.detail.sourceFrame;
 const actual=new Map();root.traverse(m=>{if(m.isMesh)actual.set(m.userData.detail.sourcePartId,m)});
 for(const file of ['body-skeleton-light.glb.gz',...region.files]){
