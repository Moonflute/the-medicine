...
export const structureClinicalOverrides: Record<string, NeuroNoteSection> = {
  ...
  "optic-chiasm": clinical([
    { label: "?? ??", text: "?? temporal visual field? ??? ??? bitemporal hemianopia? ??? ? ????." },
    { label: "?? ??", text: "confrontation visual field? formal perimetry? ?? temporal field defect? ?????." },
    { label: "???", text: "midline chiasmal compression? crossing nasal retinal fiber? ???? pituitary region lesion? ??? ?????." },
    { label: "???", text: "?? ??? refractive error? ?? ?????? ???? ?? visual acuity? fundus finding? ?? ?????." },
  ]),
  "optic-radiation": clinical([
    { label: "?? ??", text: "retrochiasmal lesion?? contralateral homonymous visual field defect? ??? ? ????." },
    { label: "???", text: "temporal lobe? Meyer loop involvement? contralateral superior quadrantanopia, parietal pathway involvement? contralateral inferior quadrantanopia? ?????." },
    { label: "?? ??", text: "visual field defect? congruity? ?? language, sensory, memory sign? ?? ?????." },
  ]),
};
