...
import { buildLabImgOverviewGroups } from "@/lib/lab-img-overview";
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="rounded-lg border border-teal-100 bg-gradient-to-br from-teal-50 to-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">세부 항목 overview의 상·하한값 표를 한 화면에 모았습니다. 참고범위는 검사실, 연령, 성별, 임신 여부와 임상 상황에 따라 달라질 수 있으므로 실제 결과지와 기관 기준을 우선합니다.</p>
      </header>
      <div className="space-y-5">
        {groups.map((group) => (
          <section key={group.title} className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 text-sm"><thead className="bg-white"><tr className="text-left text-slate-500"><th className="px-4 py-3 font-medium">Item</th><th className="px-4 py-3 font-medium text-sky-700">Low</th><th className="px-4 py-3 font-medium text-rose-700">High</th></tr></thead><tbody className="divide-y divide-slate-200 bg-white">{group.rows.map((row) => <tr key={`${group.title}-${row.slug}-${row.title}`}><td className="px-4 py-3 font-medium text-slate-950"><Link href={`/lab-img/${row.slug}`} className="inline-flex items-center gap-1 transition hover:text-teal-700">{row.title}<ChevronRight className="h-3.5 w-3.5" /></Link></td><td className="px-4 py-3 font-medium text-sky-700">{row.lower || "-"}</td><td className="px-4 py-3 font-medium text-rose-700">{row.upper || "-"}</td></tr>)}</tbody></table></div>
          </section>
        ))}
      </div>
    </div>
