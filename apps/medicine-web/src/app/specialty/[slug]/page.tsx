...
function SpecialtyRoadmapSection({ roadmap, collapsible = false }: { roadmap: NonNullable<ReturnType<typeof getSpecialtyRoadmap>>; collapsible?: boolean }) {
  const sources = (
    <div className="flex flex-wrap gap-2">
      {roadmap.sources.map((source) => (
        <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-teal-300 hover:bg-teal-50 hover:text-teal-800">
          {source.label}
        </a>
      ))}
    </div>
        ...
        <div key={lane.title} className="space-y-3">
          ...
          <div className="overflow-x-auto pb-2">
            <div className="relative flex min-w-[760px] gap-3 pr-2">
              ...
              {lane.items.map((item) => (
                <article key={[lane.title, item.time, item.title].join("-")} className="relative z-10 w-56 shrink-0 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
  ...
  if (collapsible) {
    return (
      <details className="rounded-lg border border-slate-200 bg-white/90 shadow-sm">
        <summary className="cursor-pointer list-none p-5 [&::-webkit-details-marker]:hidden">{header}</summary>
