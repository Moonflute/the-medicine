...
import { Expand, Filter, Network, RotateCcw, Search, ZoomIn, ZoomOut } from "lucide-react";
...
export function InfectionRelationMap({ pathways, spectrum }: { pathways: InfectionPathwayDataset; spectrum: AntibioticSpectrumDataset }) {
  ...
  return <div className={fullScreen ? "fixed inset-0 z-[80] overflow-auto bg-slate-950 p-3 sm:p-6" : "space-y-5"}>
    <section className={`rounded-2xl border p-4 shadow-sm sm:p-5 ${fullScreen ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-teal-700"><Network className="h-4 w-4" />Infection map</div><h2 className={`mt-1 text-xl font-bold ${fullScreen ? "text-white" : "text-slate-950"}`}>질환 · 병원체 · 항생제 관계도</h2><p className={`mt-1 text-xs ${fullScreen ? "text-slate-300" : "text-slate-500"}`}>노드를 선택하면 직접 연결 관계를 강조합니다.</p></div><div className="flex gap-2"><button type="button" onClick={() => zoom(1.2)} className="rounded-lg border border-slate-300 bg-white p-2 text-slate-700" aria-label="확대"><ZoomIn className="h-4 w-4" /></button><button type="button" onClick={() => zoom(.82)} className="rounded-lg border border-slate-300 bg-white p-2 text-slate-700" aria-label="축소"><ZoomOut className="h-4 w-4" /></button><button type="button" onClick={fit} className="rounded-lg border border-slate-300 bg-white p-2 text-slate-700" aria-label="화면 맞춤"><RotateCcw className="h-4 w-4" /></button><button type="button" onClick={() => setFullScreen((value) => !value)} className="rounded-lg border border-slate-300 bg-white p-2 text-slate-700" aria-label="전체 화면"><Expand className="h-4 w-4" /></button></div></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(220px,1.4fr)_repeat(4,minmax(130px,1fr))_auto]"><label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="질환·병원체·항생제 검색" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-3 text-sm text-slate-950 outline-none focus:border-teal-500" /></label><select value={site} onChange={(event) => setSite(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">모든 감염 부위</option>{sites.map((item) => <option key={item} value={item}>{SITE_LABELS[item] ?? item}</option>)}</select><select value={setting} onChange={(event) => setSetting(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">모든 감염 환경</option>{Object.entries(SETTING_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={population} onChange={(event) => setPopulation(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">모든 대상군</option>{Object.entries(POPULATION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={drugClass} onChange={(event) => setDrugClass(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"><option value="">모든 항생제 계열</option>{classes.map((item) => <option key={item}>{item}</option>)}</select><button type="button" onClick={reset} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-600"><Filter className="h-4 w-4" />초기화</button></div>
      <div className="mt-3 flex flex-wrap items-center gap-3 text-xs"><button type="button" onClick={() => setShowCoverage((value) => !value)} className={`rounded-full border px-3 py-1.5 font-semibold ${showCoverage ? "border-violet-700 bg-violet-700 text-white" : "border-violet-200 bg-violet-50 text-violet-900"}`}>병원체 → 항생제 활성 관계 {showCoverage ? "표시 중" : "표시"}</button><span className={fullScreen ? "text-slate-300" : "text-slate-500"}>질환 {graph.totalPathways}개 · 노드 {graph.nodes.length}개 · 관계 {graph.edgeCount}개</span></div>
    </section>
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_290px]">
      <section className="relative min-h-[540px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-sm sm:min-h-[660px]"><div className="absolute inset-0"><div ref={containerRef} className="h-full w-full" aria-label="질환 병원체 항생제 관계도" /></div><div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-slate-200 bg-white/95 px-3 py-2 text-[11px] text-slate-600 shadow-sm"><span className="mr-2 inline-block h-2.5 w-2.5 rounded-sm border border-sky-600 bg-sky-100" />질환 <span className="ml-2 mr-2 inline-block h-2.5 w-2.5 rounded-full border border-teal-700 bg-teal-50" />병원체 <span className="ml-2 mr-2 inline-block h-2.5 w-2.5 rounded-sm border border-amber-600 bg-amber-100" />항생제</div></section>
