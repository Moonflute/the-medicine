...
export function InfectionRelationMap({ pathways, spectrum }: { pathways: InfectionPathwayDataset; spectrum: AntibioticSpectrumDataset }) {
  ...
  return <div className={fullScreen ? "fixed inset-0 z-[80] overflow-auto bg-slate-950 p-3 sm:p-6" : "space-y-5"}>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_290px]">
      <section className="relative min-h-[540px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-sm sm:min-h-[660px]"><div ref={containerRef} className="absolute inset-0" aria-label="질환 병원체 항생제 관계도" /><div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-slate-200 bg-white/95 px-3 py-2 text-[11px] text-slate-600 shadow-sm"><span className="mr-2 inline-block h-2.5 w-2.5 rounded-sm border border-sky-600 bg-sky-100" />질환 <span className="ml-2 mr-2 inline-block h-2.5 w-2.5 rounded-full border border-teal-700 bg-teal-50" />병원체 <span className="ml-2 mr-2 inline-block h-2.5 w-2.5 rounded-sm border border-amber-600 bg-amber-100" />항생제</div></section>
