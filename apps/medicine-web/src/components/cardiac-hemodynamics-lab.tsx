...
import { GuidedExperiments, MetricComparison, useSimulationPeriod } from "@/components/simulation-workbench";
import { physiologyExperiments } from "@/lib/physiology-experiments";

import { useMemo, useState } from "react";
import { CardiacPhaseInspector } from "@/components/cardiac-phase-inspector";
...
export function CardiacHemodynamicsLab() {
  const { inputs, update: updateModel, animateTo } = useAnimatedModel(NORMAL); const state = useMemo(() => calculateHemodynamics(inputs), [inputs]); const [open, setOpen] = useState(false);
  useSimulationPeriod(60 / inputs.heartRate);
  const [presetId, setPresetId] = useState("normal");
  const update = (key: keyof HemodynamicsInputs, value: number) => { setPresetId("custom"); updateModel(key,value); };
  const applyPreset = (id: string) => { setPresetId(id); const preset = PRESETS.find((p) => p.id === id)!; setOpen(preset.values.heartRate !== NORMAL.heartRate); animateTo(preset.values); };
