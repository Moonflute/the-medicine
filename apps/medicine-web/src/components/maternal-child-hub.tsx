...
import { Check, ChevronRight, ExternalLink, HeartPulse, Search, ShieldAlert, Stethoscope } from "lucide-react";
...
function resolveRelated(diseases: DiseaseNote[], terms: string[]) {
  const synonyms: Record<string, string> = { "급성중이염": "중이염", "우울증": "주요우울장애" };
  return terms.map((term) => {
    const target = synonyms[normalized(term)] ?? normalized(term);
    const found = diseases.find((note) => normalized(note.title) === target || note.aliases.some((alias) => normalized(alias) === target));
    return { title: term, slug: found?.slug };
  });
}
...
export function MaternalChildHub({ diseases, hub }: { diseases: DiseaseNote[]; hub: MaternalChildHubData }) {
  ...
  const [query, setQuery] = useHubState("maternal-child-hub:query", "");
  const deferredQuery = useDeferredValue(query.trim().toLowerCase());
  const sourceById = new Map(sourceLinks.map((source) => [source.id, source]));
  const visible = stages.filter((stage) => {
    if (!(filter === "all" || stage.group === filter || stage.group === "shared")) return false;
    if (!deferredQuery) return true;
    return [stage.time, stage.title, stage.subtitle, ...stage.development, ...stage.assessments, ...stage.clinicalFocus, ...stage.related].join(" ").toLowerCase().includes(deferredQuery);
  });
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const requested = params.get("scope");
    if (requested === "all" || requested === "obstetrics" || requested === "pediatrics") setFilter(requested);
  }, [setFilter]);
  const selectFilter = (next: typeof filter) => { setFilter(next); const params = new URLSearchParams(window.location.search); params.set("scope", next); window.history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`); };
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="rounded-xl border border-rose-100 bg-white/90 p-1.5 shadow-sm" aria-label="표시 범위">
            {[{ key: "all", label: "전체" }, { key: "obstetrics", label: "산과" }, { key: "pediatrics", label: "소아" }].map((item) => (
              <button key={item.key} type="button" onClick={() => selectFilter(item.key as typeof filter)} className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold transition ${filter === item.key ? "bg-rose-700 text-white shadow-sm" : "text-slate-600 hover:bg-rose-50"}`}>
      ...
      <HubStatusStrip tone="rose" reviewedAt={hub.updatedAt} items={[{ label: "진료 단계", value: stages.length }, { label: "발달 이정표", value: pediatricMilestones.length }, { label: "출처", value: sourceLinks.length }]} />

      <label className="relative block max-w-2xl"><span className="sr-only">산부소아 Hub 검색</span><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="연령, 검사, 증상 또는 질환 검색" className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm outline-none focus:border-rose-500" /></label>
      ...
      <section aria-label="임신부터 소아청소년기까지의 타임라인" className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        <div className="relative ml-2 border-l-2 border-slate-200 pl-6 sm:ml-4 sm:pl-9">
          {visible.map((stage) => {
            ...
            return <article key={stage.id} id={stage.id} className="relative scroll-mt-6 pb-7 last:pb-0"><span className={`absolute -left-[35px] top-5 h-4 w-4 rounded-full ring-4 sm:-left-[47px] ${dot}`} aria-hidden="true" />
              <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">{links.map((link) => <Link key={link.title} href={link.slug ? `/disease/${link.slug}` : `/search?q=${encodeURIComponent(link.title)}`} className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 transition hover:border-rose-300 hover:text-rose-800">{link.title}<ChevronRight className="h-3.5 w-3.5" /></Link>)}</div>
                <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500"><span className="font-semibold">근거</span>{stage.sourceIds.map((id) => { const source = sourceById.get(id); return source ? <a key={id} href={source.url} target="_blank" rel="noreferrer" className="underline decoration-slate-300 underline-offset-2 hover:text-rose-800">{source.label}</a> : null; })}<a href={`#${stage.id}`} className="ml-auto font-semibold text-rose-700">이 단계 링크</a></div>
              </div>
            </article>;
          })}{visible.length === 0 ? <p className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">검색 조건에 맞는 단계가 없습니다.</p> : null}
        </div>
      </section>
