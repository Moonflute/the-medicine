...
function splitAssociatedItem(item: HistoryChecklistItem): HistoryChecklistItem[] {
  const symptomText = item.text.includes(":") ? item.text.slice(item.text.lastIndexOf(":") + 1) : item.text;
  const parts = symptomText.split(/\s*(?:,|;|\/|\u00b7)\s*/).map((part) => part.trim()).filter(Boolean);
  return (parts.length > 1 ? parts : [symptomText.trim() || item.text]).map((text, index) => ({ ...item, id: `${item.id}:symptom:${index}`, text }));
}
...
function buildAssociatedGroups(items: HistoryChecklistItem[]) { const grouped = new Map<string, HistoryChecklistItem[]>(); items.filter((item) => item.key === "associated").flatMap(splitAssociatedItem).forEach((item) => { const label = associatedSystemFor(item.text); grouped.set(label, [...(grouped.get(label) ?? []), item]); }); return Array.from(grouped, ([label, groupItems]) => ({ label, items: groupItems })); }
...
function HistoryChecklist({ note }: { note: ChiefComplaintNote }) {
  const [checked, setChecked] = useState<Record<string, boolean>>({}); const [showExpansions, setShowExpansions] = useState(false); const sourceItems = useMemo(() => allHistoryItems(note), [note]); const checkableItems = useMemo(() => sourceItems.flatMap((item) => item.key === "associated" ? splitAssociatedItem(item) : [item]), [sourceItems]); const associatedGroups = useMemo(() => buildAssociatedGroups(sourceItems), [sourceItems]);
  ...
  const CompactFlowCard = ({ flow }: { flow: { key: string; label: string; title: string; cue: string } }) => { const items = byKey(flow.key); return <section className="min-w-0 rounded-md border border-slate-200 bg-slate-50/70 p-2"><div className="flex items-baseline gap-1.5"><span className="text-sm font-bold text-teal-800">{flow.label}</span><h5 className="text-xs font-semibold text-slate-800">{flow.title}</h5></div>{items.length > 0 ? <div className="mt-1.5 space-y-1.5">{items.map((item) => <HistoryChip key={item.id} item={item} />)}</div> : null}</section>; };
  return <div className="mt-3 space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-3"><div className="text-sm text-slate-600">{"\uBB38\uC9C4 \uD655\uC778"} <span className="font-semibold text-slate-950">{checkableItems.filter((item) => checked[item.id]).length}</span> / {checkableItems.length}</div><div className="flex items-center gap-2"><button type="button" onClick={() => setShowExpansions((value) => !value)} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-950"><BookOpenText size={14} aria-hidden="true" />{showExpansions ? "\uC57D\uC5B4 \uD480\uC774 \uC228\uAE30\uAE30" : "\uC57D\uC5B4 \uD480\uC774"}</button><button type="button" onClick={() => setChecked({})} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-950"><RotateCcw size={14} aria-hidden="true" />{"\uCD08\uAE30\uD654"}</button></div></div>
    ...
    <details className="rounded-lg border border-slate-200 bg-white"><summary className="cursor-pointer px-3 py-2.5 text-sm font-medium text-slate-700">{"\uC804\uCCB4 \uBB38\uC9C4"}</summary><div className="space-y-4 border-t border-slate-100 p-3">{(note.historyChecklist ?? []).map((slot, slotIndex) => { const flow = COMMON_HISTORY_FLOW.find((item) => item.key === slot.key); return <section key={`${slot.key}-${slotIndex}`}><h5 className="text-xs font-semibold text-slate-800">{flow?.label ?? slot.label}</h5><div className="mt-2 space-y-2">{slot.groups.map((group, groupIndex) => <div key={`${slot.key}-${group.label}-${groupIndex}`}>{group.label !== "CC-specific" && <div className="mb-1 text-[11px] font-medium text-slate-500">{group.label}</div>}<div className="space-y-1.5">{group.items.flatMap((text, itemIndex) => { const item = { id: `history-${slotIndex}-${groupIndex}-${itemIndex}`, text, key: slot.key }; return slot.key === "associated" ? splitAssociatedItem(item) : [item]; }).map((item) => <HistoryChip key={item.id} item={item} />)}</div></div>)}</div></section>; })}</div></details>
  </div>;
}
