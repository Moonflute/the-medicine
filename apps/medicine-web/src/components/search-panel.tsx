  ...
  return (
    <section className={`w-full ${className}`.trim()}>
      <label className="surface flex items-center gap-3 px-4 py-3 focus-within:border-teal-600 focus-within:ring-2 focus-within:ring-teal-600/15 sm:px-5 sm:py-4">
        ...
        <input ref={inputRef} type="text" value={query} onChange={(event) => updateQuery(event.target.value)} onKeyDown={handleInputKeyDown} placeholder="예: 가슴 통증, STEMI, metformin" className="min-w-0 flex-1 bg-transparent text-base text-slate-950 outline-none placeholder:text-slate-400 sm:text-lg" autoFocus />
      ...
      {!term && recentSearches.length > 0 ? (
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-2">{recentSearches.map((item) => <button key={item} type="button" onClick={() => updateQuery(item)} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 hover:border-teal-300 hover:text-teal-800">{item}</button>)}</div>
        </div>
      ) : null}
