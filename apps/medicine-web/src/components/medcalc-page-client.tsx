...
function CorrectedCalciumCard() {
  ...
  const corrected =
    calciumValue !== null && albuminValue !== null ? calciumValue + 0.8 * (4 - albuminValue) : null;
  ...
  return (
    <Card title="Corrected Calcium" subtitle="Electrolytes">
      ...
      <ResultBox
        label="Result"
        value={corrected === null ? "-" : `${formatNumber(corrected)} mg/dL`}
    ...
    <Card title="Anion Gap" subtitle="Acid-Base">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <ResultBox
          ...
          value={correctedAg === null ? "-" : `${formatNumber(correctedAg)} mEq/L`}
          note="Corrected AG = AG + 2.5 × (4 - albumin)"
        />
      </div>
    </Card>
  );
...
function SodiumCorrectionCard() {
  ...
  const baseCorrection =
    na !== null && glucoseValue !== null ? na + 1.6 * ((glucoseValue - 100) / 100) : null;
  const hillierCorrection =
    na !== null && glucoseValue !== null ? na + 2.4 * ((glucoseValue - 100) / 100) : null;
  ...
  return (
    <Card title="Corrected Sodium in Hyperglycemia" subtitle="Electrolytes">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <ResultBox label="Katz 1.6" value={baseCorrection === null ? "-" : `${formatNumber(baseCorrection)} mmol/L`} />
        <ResultBox label="Hillier 2.4" value={hillierCorrection === null ? "-" : `${formatNumber(hillierCorrection)} mmol/L`} />
      </div>
    </Card>
  );
}
...
function OsmolalityCard() {
  ...
  const calculated =
    na !== null && glucoseValue !== null && bunValue !== null ? 2 * na + glucoseValue / 18 + bunValue / 2.8 : null;
...
function FenaCard() {
  ...
  const fena =
    uNa !== null && sNa !== null && uCr !== null && sCr !== null && sNa !== 0 && uCr !== 0
      ? (uNa * sCr * 100) / (sNa * uCr)
      : null;
  ...
  return (
    <Card title="FENa" subtitle="Renal">
      ...
      <ResultBox
        label="Result"
        value={fena === null ? "-" : `${formatNumber(fena)} %`}
        note="FENa = (Urine Na × Serum Cr) / (Serum Na × Urine Cr) × 100"
      />
    </Card>
  );
}
...
function FeUreaCard() {
  ...
  const feUrea =
    uUrea !== null && sUrea !== null && uCr !== null && sCr !== null && sUrea !== 0 && uCr !== 0
      ? (uUrea * sCr * 100) / (sUrea * uCr)
      : null;
  ...
  return (
    <Card title="FEUrea" subtitle="Renal">
      ...
      <ResultBox
        label="Result"
        value={feUrea === null ? "-" : `${formatNumber(feUrea)} %`}
        note="FEUrea = (Urine urea × Serum Cr) / (Serum urea × Urine Cr) × 100"
      />
    </Card>
  );
}
...
function AaGradientCard() {
  ...
  const gradient =
    fio2Value !== null && pao2Value !== null && paco2Value !== null
      ? fio2Value * (760 - 47) - paco2Value / 0.8 - pao2Value
      : null;
  ...
  return (
    <Card title="A-a Gradient" subtitle="Respiratory">
      ...
      <ResultBox
        label="Result"
        value={gradient === null ? "-" : `${formatNumber(gradient)} mmHg`}
        note="Sea-level alveolar gas equation using R = 0.8"
      />
    </Card>
  );
}
...
function Curb65Card() {
  ...
  if (score === 2) note = "Consider short admission / close review";
    ...
    <div className="space-y-6">
      <header className="rounded-[32px] border border-stone-200 bg-white/80 p-6 shadow-sm backdrop-blur sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-stone-600">
          Lab, acid-base, renal, respiratory, and bedside score calculators to keep the most common number work in one place.
        </p>
      </header>
      ...
      <section className="rounded-[28px] border border-stone-200 bg-white/85 p-5 shadow-sm sm:p-6">
        <div className="text-xs uppercase tracking-[0.22em] text-stone-500">Next Candidates</div>
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {[
            ...
            <div key={item} className="rounded-2xl border border-stone-200 bg-stone-50/70 px-4 py-3 text-sm font-medium text-stone-700">
              {item}
            </div>
          ))}
        </div>
