...
export function NervousSystemHub({ atlas, diseaseHrefs = {} }: { atlas: NeuroAtlas; diseaseHrefs?: Record<string, string>; drugHrefs?: Record<string, string> }) {
  ...
  const views = useMemo(() => atlas.views.filter((item) => item.published && item.hierarchy.length > 0), [atlas.views]);
  ...
  const updateUrl = (updates: Record<string, string | undefined>) => { const params = new URLSearchParams(window.location.search); Object.entries(updates).forEach(([key, value]) => value ? params.set(key, value) : params.delete(key)); window.history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`); };
  const selectTab = (next: Tab) => { setTab(next); updateUrl({ tab: next }); };
  const reset = () => { setZoom(1); setPan({ x: 0, y: 0 }); };
  const chooseView = (id: string) => { setPathPlaying(false); setLesionOpen(false); setViewId(id); setPathwayId(""); setPathwayStageIndex(undefined); updateUrl({ tab: "atlas", view: id, pathway: undefined }); reset(); setPickerOpen(false); };
  const chooseStructure = (id: string) => { setPathPlaying(false); setLesionOpen(false); setSelectedId(id); setPathwayId(""); setPathwayStageIndex(undefined); updateUrl({ tab: "atlas", structure: id, pathway: undefined }); };
  const choosePathway = (id: string) => { setPathPlaying(false); setLesionOpen(false); const pathway = atlas.pathways.find((item) => item.id === id); setPathwayId(id); setPathwayStageIndex(undefined); if (pathway?.nodes?.[0]) setSelectedId(pathway.nodes[0]); const nextView = pathway ? imageAtlasViewForPathway(pathway.id) : undefined; if (nextView) setViewId(nextView); updateUrl({ tab: "atlas", pathway: id || undefined, view: nextView }); reset(); setPickerOpen(false); };
  const choosePathwayStage = (index: number) => { setPathPlaying(false); const segment = selectedPathway?.segments?.[index]; if (!segment) return; setPathwayStageIndex(index); setSelectedId(segment.structureId); const nextView = imageAtlasViewForStructure(segment.structureId, viewId); if (nextView) setViewId(nextView); };
  useEffect(() => {
    if (!pathPlaying || !selectedPathway?.segments?.length) return;
    const segments = selectedPathway.segments;
    const timer = window.setInterval(() => {
      setPathwayStageIndex((previous) => {
        const next = previous === undefined ? 0 : Math.min(previous + 1, segments.length - 1);
        return next;
      });
    }, 2200 / pathSpeed);
    return () => clearInterval(timer);
  }, [pathPlaying, pathSpeed, selectedPathway]);
  useEffect(() => {
    if (!pathPlaying || pathwayStageIndex === undefined) return;
    const segment = selectedPathway?.segments?.[pathwayStageIndex];
    if (!segment) return;
  ...
  return <main className="mx-auto w-full max-w-[1540px] space-y-5 px-3 pb-24 pt-4 sm:px-5 lg:px-7">
    ...
    {tab === "atlas" ? <section className="space-y-4"><section className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:grid-cols-[minmax(280px,1fr)_minmax(260px,.9fr)_auto] md:items-end"><div><p className="mb-1.5 text-xs font-bold tracking-[.13em] text-slate-500">지도 선택</p><button type="button" onClick={() => setPickerOpen(true)} className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 px-3 py-2.5 text-left text-sm font-semibold text-slate-800 hover:border-teal-500"><span className="truncate">{selectedPathway ? `경로 · ${selectedPathway.ko}` : `${view.hierarchy.join(" › ")} · ${view.label}`}</span><ChevronRight className="h-4 w-4 shrink-0" /></button></div><label className="relative block"><span className="mb-1.5 block text-xs font-bold tracking-[.13em] text-slate-500">구조 검색</span><Search className="pointer-events-none absolute bottom-3 left-3 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="예: 시상, thalamus" className="w-full rounded-xl border border-slate-200 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-teal-600" />{query ? <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">{matches.map((item) => <button key={item.id} type="button" onClick={() => { chooseStructure(item.id); const targetView = imageAtlasViewForStructure(item.id, viewId); if (targetView) setViewId(targetView); setQuery(""); }} className="flex w-full items-center justify-between px-3 py-3 text-left hover:bg-teal-50"><span><b className="text-sm text-slate-950">{item.ko}</b><span className="ml-2 text-xs text-slate-500">{item.en}</span></span><ChevronRight className="h-4 w-4 text-slate-400" /></button>)}</div> : null}</label><button type="button" onClick={() => setFullScreen(true)} className="inline-flex h-[42px] items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-bold text-slate-700 hover:border-teal-500"><Expand className="h-4 w-4" />전체 화면</button></section>
      <section className="rounded-2xl border border-teal-200 bg-teal-50 p-4" aria-label="신경 경로 단계별 학습"><div className="flex flex-wrap items-center gap-3"><label className="flex min-w-0 flex-1 flex-col gap-1 text-xs font-bold text-teal-800">따라갈 경로<select aria-label="따라갈 신경 경로" value={pathwayId} onChange={e=>choosePathway(e.target.value)} className="max-w-full rounded-lg border border-teal-200 bg-white p-3 text-sm text-slate-900"><option value="">경로 선택</option>{atlas.pathways.filter(p=>p.segments?.length).map(p=><option key={p.id} value={p.id}>{p.ko}</option>)}</select></label>{selectedPathway?.segments?.length ? <><button type="button" className="simulation-primary" onClick={()=>{if(pathPlaying){setPathPlaying(false);return;}if(pathwayStageIndex===undefined||pathwayStageIndex===selectedPathway.segments!.length-1)setPathwayStageIndex(0);setPathPlaying(true);}}>{pathPlaying?"경로 일시정지":"경로 따라가기"}</button><button type="button" className="simulation-button" onClick={()=>choosePathwayStage(Math.max(0,(pathwayStageIndex??0)-1))}>이전 단계</button><button type="button" className="simulation-button" onClick={()=>choosePathwayStage(Math.min(selectedPathway.segments!.length-1,(pathwayStageIndex??-1)+1))}>다음 단계</button><select aria-label="경로 재생 속도" value={pathSpeed} onChange={e=>setPathSpeed(Number(e.target.value))} className="rounded-lg border bg-white p-2">{[.5,1,2].map(n=><option value={n} key={n}>{n}×</option>)}</select></>:null}</div><p className="mt-3 text-xs leading-5 text-slate-600">출발 → 중계 → 교차 → 도착을 차례로 따라갑니다. 단계 전환 속도는 신경 전도 속도가 아니며, 표식은 구조의 위치를 가리킵니다.</p>{selectedPathway && <div className="mt-4 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-white p-4"><p className="text-xs font-bold text-teal-700">현재 단계 {pathwayStageIndex===undefined?"전체":`${pathwayStageIndex+1} / ${selectedPathway.segments?.length}`}</p><h2 className="mt-1 font-bold">{pathwayStageIndex===undefined?selectedPathway.ko:selected.ko}</h2><p className="mt-2 text-sm leading-6">{pathwayStageIndex===undefined?selectedPathway.ko:selectedPathway.segments?.[pathwayStageIndex]?.label}</p><p className="mt-2 text-sm leading-6 text-slate-600">{pathwayStageIndex===undefined?selectedPathway.route:selected.summary}</p></div><div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 className="font-bold">이 경로가 손상되면?</h3><p className="mt-2 text-sm leading-6">교차 위치를 먼저 찾고, 손상된 쪽과 증상이 나타날 쪽의 관계를 예상해 보세요.</p><button type="button" className="simulation-button mt-3" onClick={()=>{setPathPlaying(false);setLesionOpen(v=>!v);}}>{lesionOpen?"해석 접기":"교차 · 병변 해석 확인"}</button>{lesionOpen&&<div className="mt-3 space-y-2 text-sm leading-6"><p><b>교차:</b> {selectedPathway.decussation??"경로 노트에서 확인"}</p><p>{selectedPathway.laterality?.description}</p><p>{selectedPathway.pattern}</p></div>}</div></div>}</section>
