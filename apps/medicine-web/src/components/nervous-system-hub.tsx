...
export function NervousSystemHub({ atlas, diseaseHrefs = {}, drugHrefs = {} }: { atlas: NeuroAtlas; diseaseHrefs?: Record<string, string>; drugHrefs?: Record<string, string> }) {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      setZoom((value) => Math.max(.75, Math.min(2.4, value + (event.deltaY < 0 ? .1 : -.1))));
    };
    ...
    {tab === "structure" ? <section className="space-y-4">
      ...
      <div className={"grid gap-4 " + (desktopInfoOpen ? "xl:grid-cols-[minmax(0,1fr)_330px]" : "xl:grid-cols-1")}>
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div ref={atlasWheelRef} className="relative min-h-[460px] overflow-hidden bg-slate-50 sm:min-h-[600px]" onPointerDown={onAtlasPointerDown} onPointerMove={onAtlasPointerMove} onPointerUp={onAtlasPointerEnd} onPointerCancel={onAtlasPointerEnd}>
