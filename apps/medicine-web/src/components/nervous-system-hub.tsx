...
import { imagePilotViewIds } from "@/components/image-neuro-atlas";
...
export function NervousSystemHub({ atlas, diseaseHrefs = {} }: { atlas: NeuroAtlas; diseaseHrefs?: Record<string, string> }) {
  ...
  return <main className="mx-auto w-full max-w-[1540px] space-y-5 px-3 pb-24 pt-4 sm:px-5 lg:px-7">
    ...
    {tab === "nex" ? <section className="space-y-4">
      ...
      <aside className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-5 xl:self-start"><p className="text-xs font-bold uppercase tracking-[.14em] text-teal-700">Examination notes</p><p className="mt-3 text-sm leading-6 text-slate-700"><b>Route:</b> {reflex.arc}</p>{reflex.technique?.length ? <ul className="mt-4 list-disc space-y-1 pl-5 text-sm leading-6 text-slate-700">{reflex.technique.map((item) => <li key={item}>{item}</li>)}</ul> : null}<dl className="mt-4 grid gap-3 border-t border-slate-100 pt-4 text-sm leading-6"><div><dt className="font-bold text-slate-950">Normal response</dt><dd className="mt-1 text-slate-700">{reflex.normal ?? "Interpret the response in the clinical context."}</dd></div><div><dt className="font-bold text-slate-950">Abnormal findings / cautions</dt><dd className="mt-1 text-slate-700">{reflex.abnormal ?? "Compare both sides and interpret with the rest of the examination."}</dd></div><div><dt className="font-bold text-slate-950">Localization</dt><dd className="mt-1 text-slate-700">{reflex.localization}</dd></div>{reflex.laterality ? <div><dt className="font-bold text-slate-950">Laterality</dt><dd className="mt-1 text-slate-700">{reflex.laterality.description}</dd></div> : null}</dl><button type="button" onClick={() => { const route = reflex.route ?? []; const target = [...route.slice(nexStage), ...route.slice(0, nexStage).reverse()].find((id) => structures.some((structure) => structure.id === id)); setTab("structure"); setLayer("reflex"); setPathwayId("reflex:" + reflex.id + ":" + nexStage); setViewId(reflex.viewId && imagePilotViewIds.has(reflex.viewId) ? reflex.viewId : "whole-neuraxis"); if (target && structures.some((structure) => structure.id === target)) setSelectedId(target); reset(); }} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-bold text-white"><Focus className="h-4 w-4" />Show selected stage in Atlas</button></aside></section> : null}
    </section> : null}
