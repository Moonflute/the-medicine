...
export function AudioReviewClient({ catalog }: { catalog: AudioDocument[] }) {
  ...
  const categoryLabel = categories.length === 0 ? "전체 분류" : String(categories.length) + "개 분류 선택";
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          <div><h3 className="text-sm font-semibold text-slate-800">1. 범주</h3><div className="mt-2 flex flex-wrap gap-2">{DOMAIN_ORDER.map((domain) => <button key={domain} type="button" onClick={() => toggleDomain(domain)} className={`rounded-lg border px-3 py-2 text-sm font-medium ${domains.includes(domain) ? "border-teal-500 bg-teal-600 text-white" : "border-slate-200 bg-white text-slate-700"}`}>{DOMAIN_LABELS[domain]}</button>)}</div><div className="mt-3"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => setCategoriesOpen((open) => !open)} className="inline-flex h-10 min-w-36 items-center justify-between gap-3 rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 transition hover:border-teal-300 hover:text-teal-800"><span>{categoryLabel}</span>{categoriesOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>{categories.length > 0 ? <button type="button" onClick={clearCategories} className="h-10 rounded-lg px-3 text-sm font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-800">전체로</button> : null}</div>{categoriesOpen ? <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">{availableCategories.map((category) => <button key={category} type="button" onClick={() => toggleCategory(category)} className={`min-h-12 w-full rounded-lg border px-3 py-2 text-left text-sm leading-5 transition ${categories.includes(category) ? "border-teal-400 bg-teal-50 font-semibold text-teal-900" : "border-slate-200 bg-white text-slate-700 hover:border-slate-300 hover:bg-slate-50"}`}><span className="block break-keep">{category}</span></button>)}</div> : null}</div></div>
