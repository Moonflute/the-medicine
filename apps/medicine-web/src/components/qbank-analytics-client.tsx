...
import { practiceTopicLabel } from "@/lib/practice-selection";
...
export function QbankAnalyticsClient() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/generated/qbank/index.json`, { signal: controller.signal }).then(r => { if (!r.ok) throw new Error(); return r.json(); }).then((items: QbankQuestionIndex[]) => {
      setPublicQuestions(items.map(q => ({ id: q.id, bank: q.questionBank, department: clean(q.specialty), topic: relatedTopic(q), label: `${relatedTopic(q)} · ${q.id}` })));
      setLoaded(true);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        setPractice(result.questions.map(q => ({ id: q.id, bank: "practice", department: q.bookDepartment, topic: practiceTopicLabel(q), label: `${q.bookSeries === "퍼펙트" ? "P" : "R"} ${q.examYear ?? "연도 미상"} · ${q.bookDepartment} · ${q.id.split("-").at(-1)?.replace(/^0+/, "") || "0"}번`, exam: `${q.bookSeries === "퍼펙트" ? "P" : "R"} ${q.examYear ?? "연도 미상"}` })));
  ...
  const departments = useMemo(() => [...new Set(catalog.filter(q => bank === "all" || q.bank === bank).map(q => q.department))].sort((a, b) => a.localeCompare(b, "ko")), [catalog, bank]);
  ...
  return <div className="space-y-5">
    ...
    <section className="surface min-w-0 p-5 sm:p-6"><h2 className="font-semibold text-slate-900">과목별 성적과 학습 범위</h2><p className="mt-2 text-xs text-slate-500">정답률과 함께 풀어본 문항 수를 확인하세요. 미풀이 문제는 정답률 계산에서 제외합니다.</p>{groupRows(stats.groups)}</section>
