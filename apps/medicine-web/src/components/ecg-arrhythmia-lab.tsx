...
import { GuidedExperiments, useSimulationClock } from "@/components/simulation-workbench";
...
export function EcgArrhythmiaLab() {
  ...
  const pick = (id: RhythmId) => { const selected = RHYTHMS.find((item) => item.id === id)!; setRhythm(id); setRate(selected.rate); updateSimulationClock(clock, {seconds: 0}); };
  ...
  return <div className="physiology-lab space-y-5">
    <PhysiologyHeader title="ECG와 부정맥" description="리듬별 실제 P파와 QRS 사건을 생성해 SA node, AV node, His-Purkinje 전도와 표면 ECG를 연결하고, 심실 활성과 기계적 반응을 구분해 관찰합니다. 맥박 표시는 유효 수축을 가정한 교육 표현입니다." links={[{ href: "/interactive/cardiac-hemodynamics", label: "심장 혈역학" }]} />
    <GuidedExperiments experiments={physiologyExperiments.ecg} onApply={(id) => pick(id as RhythmId)} />
    ...
    <section className="physiology-stage">
      <CanvasFrame legend={<div className="flex flex-wrap gap-x-5 gap-y-2"><span><b className="text-amber-700">●</b> 전도된 심방 활성</span><span><b className="text-rose-700">●</b> 차단된 P파</span><span><b className="text-violet-700">●</b> QRS와 뒤따르는 맥박</span><span>8초 고정 파형 · 커서 = 관찰 시점</span></div>}><EcgP5Canvas state={state} /></CanvasFrame>
    ...
    <section className="simulation-comparison" aria-label="전도 사건 선택"><h2 className="mb-3 font-bold">사건을 선택해서 멈춰 보기</h2><div className="flex max-h-44 flex-wrap gap-2 overflow-y-auto">{state.events.map((event, index) => <button type="button" key={index} className="simulation-button" onClick={() => { updateSimulationClock(clock, {playing: false}); updateSimulationClock(clock, {seconds: event.time + 0.02}); }}>{event.time.toFixed(2)}s · {event.type === "ventricular" ? "QRS" : rhythm === "af" ? "심방 활성" : rhythm === "flutter" ? "F" : event.conducted ? "P" : "P · 비전도"}</button>)}</div><p className="mt-3 text-xs text-slate-500">8초 예시 창 · 끝에 도달하면 예시 처음으로 돌아갑니다. 경계는 연속 리듬을 뜻하지 않으며, 맥박은 기계적 반응을 가정한 표현입니다.</p></section>
    ...
    <section className="rounded-md border border-violet-200 bg-violet-50 p-5"><div className="flex items-center gap-2 text-sm font-bold"><Activity className="h-4 w-4" />전기-기계 연결</div><p className="mt-2 text-sm leading-6 text-slate-700">P파는 심방 탈분극, QRS는 심실 탈분극을 나타냅니다. Mobitz I은 주로 AV node, Mobitz II는 주로 His–Purkinje 전도 장애와 연결하고, 완전 방실차단에서는 심방과 심실 사건이 독립적으로 생성되며, 심실 사건 뒤에만 맥박 애니메이션이 나타납니다.</p></section>
  </div>;
}
