...
export function PersonalHighlighter() {
  ...
  return <div ref={tools} className="relative shrink-0" data-highlight-ignore>
    <button type="button" aria-label="형광펜 도구" title={mode === "pen" ? "형광펜 모드 켜짐" : mode === "erase" ? "지우개 모드 켜짐" : "개인 형광펜"} aria-expanded={open} onMouseDown={event => { capture(); event.preventDefault(); }} onClick={() => setOpen(value => !value)} className={`inline-flex h-10 w-10 items-center justify-center rounded-lg border transition ${mode !== "read" && user ? "border-amber-400 bg-amber-100 text-amber-900" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}><Highlighter size={18} /></button>
    {open && <div role="dialog" aria-label="형광펜 도구 패널" className="fixed right-3 top-[4.5rem] z-50 max-h-[calc(100dvh-6rem)] w-72 max-w-[90vw] overflow-y-auto rounded-xl border border-slate-200 bg-white p-3 text-slate-800 shadow-xl sm:absolute sm:right-0 sm:top-auto sm:mt-2">
      ...
      {!user ? <p className="text-xs leading-5 text-slate-600">Google 계정으로 로그인하면 표시가 내 계정에 저장됩니다.</p> : <>
        <div className="flex justify-between gap-2" role="group" aria-label="형광펜 색상">{HIGHLIGHT_COLORS.map(value => <button key={value} type="button" aria-label={COLORS[value].name + " 형광펜"} aria-pressed={color === value} title={COLORS[value].name} onMouseDown={event => event.preventDefault()} onClick={() => { setColor(value); if (selection.current) apply(false, value); }} className={`h-8 w-8 rounded-full border-2 ${color === value ? "border-slate-700 ring-2 ring-slate-200" : "border-transparent"}`} style={{ backgroundColor: COLORS[value].swatch }} />)}</div>
        <div className="mt-3 grid grid-cols-2 gap-1">
          <button type="button" aria-pressed={mode === "pen"} className={button + (mode === "pen" ? " bg-amber-100" : "")} onMouseDown={event => event.preventDefault()} onClick={() => setMode(value => value === "pen" ? "read" : "pen")}><Highlighter size={15} />형광펜 모드</button>
          <button type="button" aria-pressed={mode === "erase"} className={button + (mode === "erase" ? " bg-slate-200" : "")} onMouseDown={event => event.preventDefault()} onClick={() => setMode(value => value === "erase" ? "read" : "erase")}><Eraser size={15} />지우개 모드</button>
          <button type="button" className={button} onMouseDown={event => event.preventDefault()} onClick={() => setMode("read")}><MousePointer2 size={15} />일반 읽기</button>
          <button type="button" className={button} onMouseDown={event => event.preventDefault()} onClick={() => apply(false)}><Highlighter size={15} />선택 부분 표시</button>
          <button type="button" className={button} onMouseDown={event => event.preventDefault()} onClick={() => apply(true)}><Eraser size={15} />선택 부분 지우기</button>
