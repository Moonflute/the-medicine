...
export function RelatedClinicalContent({
  relations,
  limitPerGroup = 12,
}: {
    ...
    <section className="rounded-lg border border-slate-200 bg-white/85 p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold text-slate-950">Related clinical content</h2>
        </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {group.items.map((relation) => (
                <Link
                  ...
                  href={relation.targetHref}
                  title={relation.evidence || relation.relation}
                  className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 transition hover:border-teal-300 hover:text-teal-800"
                >
                  <span>{relation.targetTitle}</span>
                  <span className={relation.provenance === "generated" ? "text-amber-700" : "text-teal-700"}>
                    {relation.provenance === "generated" ? "자동" : "명시"}
                  </span>
                </Link>
              ))}
            </div>
