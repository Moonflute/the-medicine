...
import { diseasesForReflex, diseasesForStructure, getNeuroNoteItem, medicalTerm, neuroNoteHref, type NeuroNoteKind, relatedStructures } from "@/lib/neuro-notes";
...
function LinkPills({ items, hrefs, empty }: { items: string[]; hrefs: Record<string, string>; empty: string }) {
  ...
  return <div className="flex flex-wrap gap-2">{linked.map((item) => <Link key={item} href={hrefs[item]} className="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-sm font-semibold text-teal-900 hover:border-teal-500 hover:bg-white">{medicalTerm(item)}<ChevronRight className="ml-1 inline h-3.5 w-3.5" /></Link>)}</div>;
}
...
export function NeuroNotePage({ atlas, kind, id, diseaseHrefs, drugHrefs }: Props) {
  ...
  if (kind === "structure") {
    ...
    return <NoteFrame icon={Icon} typeLabel={typeLabel} title={structure.en} subtitle={structure.ko} backHref="/nervous-system-hub?tab=notes" source={<Sources atlas={atlas} sourceIds={structure.sourceIds ?? []} />}>
      ...
      <Section title="Clinical relevance"><p>{structure.links.length ? `Use the linked disease notes to connect dysfunction of ${structure.en} with clinical presentation. Localize findings with the relevant pathway and neurological examination rather than from an isolated anatomical label.` : `Assess suspected involvement of ${structure.en} in the context of adjacent structures, pathway laterality, and the full neurological examination.`}</p></Section>
      <Section title="Related diseases"><LinkPills items={structure.links} hrefs={diseaseHrefs} empty="No disease note is currently mapped to this structure." /></Section>
      <Section title="Related structures"><StructureLinks structures={related} /></Section>
  ...
  if (kind === "pathway") {
    ...
    return <NoteFrame icon={Icon} typeLabel={typeLabel} title={pathway.en} subtitle={pathway.ko} backHref="/nervous-system-hub?tab=notes" source={<Sources atlas={atlas} sourceIds={pathway.sourceIds ?? []} />}>
      ...
      <Section title="Clinical localization"><p>{pathway.lesionPattern ?? pathway.pattern}</p>{pathway.laterality ? <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-950"><b>Laterality:</b> {pathway.laterality.description}</p> : null}</Section>
      <Section title="Related diseases"><LinkPills items={pathway.links} hrefs={diseaseHrefs} empty="No disease note is currently mapped to this pathway." /></Section>
  ...
  if (kind === "reflex") {
    ...
    return <NoteFrame icon={Icon} typeLabel={typeLabel} title={reflex.label} subtitle="신경학적 진찰 · 반사 회로" backHref="/nervous-system-hub?tab=nex" source={<Sources atlas={atlas} sourceIds={reflex.sourceIds ?? []} />}>
      ...
      <Section title="Clinical localization"><p>{reflex.localization}</p></Section>
      <Section title="Related diseases"><LinkPills items={diseasesForReflex(atlas, reflex.id)} hrefs={diseaseHrefs} empty="No disease note is currently mapped to this reflex circuit." /></Section>
      <Section title="Related structures"><StructureLinks structures={structures} /></Section>
      <Section title="연관 구조"><StructureLinks structures={structures} /></Section>
