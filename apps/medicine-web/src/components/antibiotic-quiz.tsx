...
type QuizType = "organism-to-drug" | "drug-to-organism" | "coverage-reading";
type Question = {
  id: string;
  type: QuizType;
  prompt: string;
...
const QUIZ_TYPES: Array<{ id: QuizType; label: string; description: string }> = [
...
function questionBank(dataset: AntibioticSpectrumDataset): Question[] {
...
function choiceLabel(dataset: AntibioticSpectrumDataset, question: Question, value: string) {
...
export function AntibioticQuiz({ dataset }: { dataset: AntibioticSpectrumDataset }) {
  ...
  const bank = questionBank(dataset);
  ...
  const toggleType = (type: QuizType) => setTypes((currentTypes) => currentTypes.includes(type) ? currentTypes.filter((item) => item !== type) : [...currentTypes, type]);
