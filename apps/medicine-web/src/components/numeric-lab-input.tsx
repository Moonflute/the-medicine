...
const AUTOIMMUNE_QUALITATIVE: QualitativeField[] = [
  { id: "antiSm", label: "Anti-Sm", options: ["negative", "trace", "positive"] }, { id: "antiRoSsa", label: "Anti-Ro/SSA", options: ["negative", "trace", "positive"] }, { id: "antiLaSsb", label: "Anti-La/SSB", options: ["negative", "trace", "positive"] }, { id: "antiScl70", label: "Anti-Scl-70", options: ["negative", "trace", "positive"] }, { id: "antiCentromere", label: "Anti-centromere", options: ["negative", "trace", "positive"] }, { id: "anticardiolipin", label: "Anticardiolipin antibody", options: ["negative", "trace", "positive"] }, { id: "beta2Gp1", label: "Anti-β2 glycoprotein I", options: ["negative", "trace", "positive"] }, { id: "lupusAnticoagulant", label: "Lupus anticoagulant", options: ["negative", "trace", "positive"] },
];
...
const SOURCE_LINKS = [
  ["ACR: ANA interpretation", "https://rheumatology.org/patients/antinuclear-antibodies-ana"], ["ACR: rheumatology classification criteria", "https://rheumatology.org/criteria"], ["ACR/VF: ANCA-associated vasculitis", "https://vasculitisfoundation.org/anca-associated-vasculitis-gpa-mpa-egpa-guidelines/"],
...
function isAutoantibodyPositive(values: Record<string, string>, key: string) { return qualitativeAt(values, key) === "positive"; }
...
function buildExpandedFindings(values: Record<string, string>, add: (finding: Finding) => void) {
  ...
  const rheumatoidFactor = value("rheumatoidFactor"), antiCcp = value("antiCcp"), anaTiter = value("anaTiter"), antiDsDnaXuln = value("antiDsDnaXuln"), c3 = value("c3"), c4 = value("c4"), pr3AncaXuln = value("pr3AncaXuln"), mpoAncaXuln = value("mpoAncaXuln");
  if (antiCcp !== undefined && antiCcp > 20 && rheumatoidFactor !== undefined && rheumatoidFactor > 20) add({ level: "attention", title: "RF-anti-CCP 동시 상승", summary: `Rheumatoid factor ${rheumatoidFactor} IU/mL와 anti-CCP ${antiCcp} U/mL가 함께 상승했습니다. 염증성 관절염 맥락에서는 류마티스 관절염을 강하게 지지할 수 있습니다.`, next: "관절 분포·활막염, ESR/CRP, 영상 및 다른 원인을 함께 평가. 항체만으로 류마티스 관절염을 확정하지 않음", diseases: ["류마티스 관절염"] });
  else if ((antiCcp !== undefined && antiCcp > 20) || (rheumatoidFactor !== undefined && rheumatoidFactor > 20)) add({ level: "pattern", title: "류마티스 관절염 관련 항체 상승", summary: `${antiCcp !== undefined && antiCcp > 20 ? "Anti-CCP" : "Rheumatoid factor"} 상승이 확인됩니다. 단독 양성은 임상 관절염 및 다른 원인과 함께 해석합니다.`, next: "대칭성 소관절 활막염·지속 기간, ESR/CRP 및 감염·다른 결합조직질환 가능성을 함께 확인", diseases: ["류마티스 관절염"] });
  const lowComplement = (c3 !== undefined && c3 < 90) || (c4 !== undefined && c4 < 10);
  if (anaTiter !== undefined && anaTiter >= 80 && ((antiDsDnaXuln !== undefined && antiDsDnaXuln > 1) || isAutoantibodyPositive(values, "antiSm")) && lowComplement) add({ level: "attention", title: "ANA-특이항체-저보체 조합", summary: "ANA 양성 범위, SLE 관련 특이항체(anti-dsDNA 또는 Sm) 및 저보체가 함께 있습니다. 전신 홍반 루푸스와 장기 침범 가능성을 임상적으로 평가하는 조합입니다.", next: "피부·관절·혈액학적 소견, 소변침사·UACR·creatinine을 함께 확인. 단백뇨/혈뇨 또는 신기능 이상이 있으면 루푸스 콩팥염 평가를 앞당김", diseases: ["전신 홍반 루푸스", "루푸스 콩팥염"] });
  else if (anaTiter !== undefined && anaTiter >= 80) add({ level: "pattern", title: "ANA 양성 범위", summary: `ANA 1:${anaTiter}입니다. ANA는 결합조직질환의 선별 단서가 될 수 있으나 단독 양성의 특이도는 제한적입니다.`, next: "증상·진찰, ENA/anti-dsDNA, 보체 및 약물·감염 맥락을 함께 확인", diseases: ["전신 홍반 루푸스", "전신 경화증"] });
  if ((pr3AncaXuln !== undefined && pr3AncaXuln > 1) || (mpoAncaXuln !== undefined && mpoAncaXuln > 1)) add({ level: "attention", title: "PR3/MPO-ANCA 상승", summary: `${pr3AncaXuln !== undefined && pr3AncaXuln > 1 ? "PR3-ANCA" : "MPO-ANCA"} 상승이 확인됩니다. ANCA 연관 혈관염을 포함해 평가하되, 항체 단독으로 진단하거나 질병활성을 판단하지 않습니다.`, next: "ENT·폐·피부·신경 증상, urine sediment·creatinine, 영상 및 필요 시 조직학적 평가를 함께 확인", diseases: ["혈관염", "급성 콩팥 손상"] });
  if (isAutoantibodyPositive(values, "antiRoSsa") || isAutoantibodyPositive(values, "antiLaSsb")) add({ level: "pattern", title: "SSA/SSB 자가항체 양성", summary: "SSA 또는 SSB 항체 양성이 입력되었습니다. 건조증·피부·관절·신경 증상과 함께 쇼그렌증후군 및 다른 결합조직질환을 평가합니다.", next: "객관적 안구/타액선 검사, ANA·RF·보체 및 약물·감염 등 다른 원인을 함께 확인", diseases: ["전신 홍반 루푸스"] });
  if (isAutoantibodyPositive(values, "antiScl70") || isAutoantibodyPositive(values, "antiCentromere")) add({ level: "pattern", title: "전신 경화증 관련 자가항체 양성", summary: "Scl-70 또는 centromere 항체 양성이 입력되었습니다. Raynaud 현상·피부경화·폐혈관/간질성 폐질환 평가와 함께 해석합니다.", next: "항체 pattern·역가, 피부·혈관 소견, PFT/DLCO·HRCT·심초음파 필요성을 임상적으로 확인", diseases: ["전신 경화증"] });
  const apsAntibodyCount = [isAutoantibodyPositive(values, "anticardiolipin"), isAutoantibodyPositive(values, "beta2Gp1"), isAutoantibodyPositive(values, "lupusAnticoagulant")].filter(Boolean).length;
  if (apsAntibodyCount >= 1) add({ level: "attention", title: "항인지질항체 양성", summary: `항인지질항체 ${apsAntibodyCount}개가 양성으로 입력되었습니다. 항인지질 증후군은 혈전 또는 임신 관련 임상 기준과 12주 이상 간격의 반복 양성이 함께 필요합니다.`, next: "혈전·임신 합병증 병력, 항체 종류/역가 및 12주 후 반복 검사를 확인", diseases: ["항인지질 증후군", "전신 홍반 루푸스"] });
