...
export function NumericLabInput({ diseases }: { diseases: DiseaseNote[] }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 xl:grid-cols-2">{NUMERIC_PANELS.map((panel) => <article key={panel.title} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3"><h2 className="font-bold text-slate-950">{panel.title}</h2><p className="mt-1 text-xs text-slate-500">{panel.description}</p></div><div className="grid grid-cols-2 gap-2">{panel.fields.map((field) => { const value = numberAt(values, field.id); const status = value === undefined ? undefined : classify(field, value); return <label key={field.id} className="min-w-0 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-2 sm:px-3"><span className="flex min-w-0 items-center justify-between gap-1.5 text-xs font-semibold text-slate-800 sm:text-sm"><span className="min-w-0 leading-4">{field.label}</span>{status ? <StatusPill status={status} /> : null}</span><span className="mt-1 flex min-w-0 items-center gap-1.5"><input inputMode="decimal" type="number" step="any" value={values[field.id] ?? ""} onChange={(event) => setValue(field.id, event.target.value)} className="min-w-0 flex-1 rounded border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-950 outline-none focus:border-teal-500" placeholder="입력" /><span className="shrink-0 text-[11px] text-slate-500 sm:text-xs">{field.unit}</span></span><span className="mt-1 block text-[11px] text-slate-500">{field.low !== undefined || field.high !== undefined ? `${field.low ?? ""}${field.low !== undefined && field.high !== undefined ? "–" : ""}${field.high ?? ""} ${field.unit}` : field.note}</span></label>; })}</div></article>)}</section>
