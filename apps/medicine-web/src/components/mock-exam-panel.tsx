...
export function MockExamPanel({ questions, exam, sessionId, currentIndex, onChange, onMove }: {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
      {(!finished || review) && <article data-highlight-document={`qbank:${current.id}`} id="mock-question" className="surface min-w-0 scroll-mt-20 p-5 sm:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className="pill">{mockSubject(current.id)}</span><span className="ml-2 text-sm font-semibold">{currentIndex + 1} / {questions.length}</span></div>{!finished && <button type="button" aria-pressed={exam.flaggedIds.includes(current.id)} className="secondary-action" onClick={() => onChange({ ...exam, flaggedIds: exam.flaggedIds.includes(current.id) ? exam.flaggedIds.filter(id => id !== current.id) : [...exam.flaggedIds, current.id] })}><Flag className="h-4 w-4" />{exam.flaggedIds.includes(current.id) ? "보류 해제" : "보류"}</button>}</div>
        <p className="mt-3 break-all text-xs text-slate-400">{current.id}</p><p data-highlight-block="question" className="mt-5 whitespace-pre-line text-[15px] leading-7">{reflowOcrText(current.question)}</p>
